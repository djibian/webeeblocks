...
(async()=>{
  ...
  var c=backend([.4,.8,.3]),pauses=[],sensors=[],obs=Observer.create(workspace(),{onPause:d=>pauses.push([d.blockId,d.node.kind]),onSensor:d=>sensors.push([d.blockId,d.direction,d.value])});
  var run=Interpreter.run(ast(),c,{maxSteps:1000,hooks:obs.begin(true)});
  await until(()=>pauses.length===1,'takeoff');assert.deepStrictEqual(c.trace,[]);obs.next();
  await until(()=>pauses.length===2,'range');assert.deepStrictEqual(c.trace,[['takeoff',1]]);assert.deepStrictEqual(pauses[1],['range-front','range']);obs.next();
  await until(()=>sensors.length===1,'sensor');await until(()=>pauses.length===3,'move');assert.deepStrictEqual(sensors[0],['range-front','front',.4]);assert.deepStrictEqual(pauses[2],['left-action','move']);obs.continueRun();await run;obs.finish();
  ...
  console.log('PASS: observability is optional, stepping is causal, raw sensor is fresh, and machine codes remain testable.');
