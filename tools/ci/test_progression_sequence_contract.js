...
assert.strictEqual(p2.world, initialProfile.world,
  'Activity 2 must reuse the shared Runtime-v2 world compatibility tag');
assert.strictEqual(p2.brief.title, '2 — Livrer le colis fragile');
assert.match(p2.brief.mission, /petite zone de livraison située à gauche du couloir/);
assert.match(p2.brief.mission, /sans toucher l’obstacle rouge/);
assert.strictEqual(p2.brief.goal, p2.brief.mission,
  'legacy presentation compatibility must derive from the Activity 2 mission');
...
assert.deepStrictEqual(p2.toolbox,
  ['webeeblocks_v2_takeoff','webeeblocks_v2_move','webeeblocks_v2_land']);
assert.deepStrictEqual(p2.fieldOptions.webeeblocks_v2_move.DIRECTION, ['forward','left']);
assert.deepStrictEqual(p2.parameterBounds.webeeblocks_v2_takeoff.HEIGHT,
  {min:0.5,max:0.5,step:0.1},
  'Activity 2 must reuse takeoff without making altitude a second new concept');
assert.deepStrictEqual(p2.parameterBounds.webeeblocks_v2_move.DISTANCE,
  {min:0.1,max:0.6,step:0.1},
  'Activity 2 must expose meaningful distance choice while staying bounded to the local mission geometry');
...
assert.match(worldSource, /name "Progression precise movement evaluator"[\s\S]*controller "crazyflie_runtime_v2"[\s\S]*"precise-evaluator-v1"/);
...
assert.match(runtimeEntrySource, /webeeblocks_progression_precise_evaluator_main/);
...
const preciseEvaluatorSource = fs.readFileSync(path.resolve(__dirname, '../../controllers/crazyflie_runtime_v2/progression_precise_evaluator.c'), 'utf8');
...
assert.match(preciseEvaluatorSource, /wb_supervisor_node_get_contact_points/,
  'Activity 2 must reject physical contact with the red obstacle from observable world state');
assert.match(preciseEvaluatorSource, /WEBEEBLOCKS_PRECISE_COLLISION/);
assert.match(preciseEvaluatorSource, /WEBEEBLOCKS_PRECISE_TIMEOUT/);
assert.doesNotMatch(preciseEvaluatorSource, /Blockly|workspace|allowedStatementKinds|webeeblocks_v2_/,
  'Activity 2 evaluator must not inspect Blockly or expected solution shape');
...
const precisePad = worldSource.match(
  /WEBEEBLOCKS_PRECISE_MOVEMENT_MISSION_V1_BEGIN[\s\S]*?translation (0\.30) (0\.20) 0\.002[\s\S]*?baseColor 0\.92 0\.72 0\.10[\s\S]*?geometry Box \{ size ([0-9.]+) ([0-9.]+) 0\.004 \}/);
assert.ok(precisePad, 'Activity 2 small visible delivery zone must remain explicit');
const preciseX = Number(preciseEvaluatorSource.match(/#define PRECISE_TARGET_X ([0-9.]+)/)[1]);
const preciseY = Number(preciseEvaluatorSource.match(/#define PRECISE_TARGET_Y ([0-9.]+)/)[1]);
const preciseXTol = Number(preciseEvaluatorSource.match(/#define PRECISE_TARGET_X_TOLERANCE ([0-9.]+)/)[1]);
const preciseYTol = Number(preciseEvaluatorSource.match(/#define PRECISE_TARGET_Y_TOLERANCE ([0-9.]+)/)[1]);
assert.strictEqual(preciseX, Number(precisePad[1]));
assert.strictEqual(preciseY, Number(precisePad[2]));
assert.strictEqual(preciseXTol, Number(precisePad[3]) / 2,
  'Activity 2 X acceptance must be exactly visible, not a hidden tighter precision boundary');
assert.strictEqual(preciseYTol, Number(precisePad[4]) / 2,
  'Activity 2 Y acceptance must be exactly visible, not a hidden tighter precision boundary');
assert.ok(Number(precisePad[3]) < padWidth && Number(precisePad[4]) < padDepth,
  'Activity 2 target must be materially smaller than the generous Activity 1 receiving pad');
...
async function exerciseEmbeddedStartActivityPath(profile) {
  ...
  const transport = {
    ...
    async open() { transportState.opens += 1; throw new Error('embedded Start activity must not open the OS project picker'); },
  ...
  try {
    ...
    assert.strictEqual(profileRef.value.brief.mission, profile.brief.mission);
    assert.strictEqual(manager.hasCurrentTarget(), false,
...
(async function() {
  ...
  console.log('PASS progression Activities 1 and 2 preserve embedded-start compatibility, explicit mission/pedagogy separation, cumulative concept scope, visible world-state evaluation and solution-shape independence while leaving later profiles unaccepted');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
