...
class ContractTests(unittest.TestCase):
    ...
    def test_stigmergic_work_dispersion_is_local_bounded_and_non_authoritative(self):
        for required in (
            ...
            "Interference describes concrete coupling in the mutation or validation surface of bounded results",
            "When current durable project state exposes such overlap, treat it as a negative strength signal",
            "never as evidence of unseen Controller presence, ownership or reservation",
            "Lesser interference is not a deterministic tie-breaker and overlap does not by itself exclude a candidate",
            "comparable strong candidates that remain are dispersed using the execution-local dispersion key",
