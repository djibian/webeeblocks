...
# Re-run the exact settled C25 harness first so its A/I/F controls, pinned Webots
# closure and build products are recreated unchanged before the C26 discriminator.
...
g++ "${common[@]}" "$control_obj" "$broker_obj" "$got_obj" \
  -Wl,--gc-sections -Wl,--undefined=webeeblocks_c26_got_self \
...
# Settled C25 controls must retain their exact structural distinction.
no_provider_roots a
! has_qcore_root a
! has_meta_root a
! has_qcore_copy a
...
# K references the same Qt data symbol but must not interpose it via COPY.
no_provider_roots k
! has_qcore_root k
! has_meta_root k
! has_qcore_copy k
...
if check_success "$k_log" arm-k >&2; then
  ...
  echo DIAGNOSTIC_RESULT=C26_COPY_RELOCATION_NECESSARY_IN_MINIMAL_CLOSURE | tee "$artifact_dir/result.txt"
  exit 0
fi
if check_crash "$k_log" arm-k >&2; then
  echo 'C26_OUTCOME got=crash copy=crash instance=crash' | tee "$artifact_dir/outcomes.txt"
  echo DIAGNOSTIC_RESULT=C26_SELF_REFERENCE_SUFFICIENT_WITHOUT_COPY | tee "$artifact_dir/result.txt"
  exit 0
fi
