...
class WindowsReleaseContractTests(unittest.TestCase):
    ...
    def test_launcher_opens_only_packaged_world_with_r2025a(self) -> None:
        ...
        self.assertIn("Loopback, $Port", launcher)
        ...
        self.assertNotIn("New-WebeeBlocksRobotWindowSession", launcher)
        self.assertNotIn("_session_${PID}_$token", launcher)
        self.assertIn(
            "Start-WebeeBlocksLocalServer -Root $PSScriptRoot -Port $WebeeBlocksLocalServerPort",
            launcher,
        )
        self.assertIn("for ($attempt = 1; $attempt -le 2; $attempt++)", launcher)
        self.assertIn("http://127.0.0.1:$WebeeBlocksLocalServerPort/", launcher)
        self.assertIn("Set-WebeeBlocksRobotWindowHtml", launcher)
        self.assertIn("$robotWindow.Html", launcher)
