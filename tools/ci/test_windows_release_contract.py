...
class WindowsReleaseContractTests(unittest.TestCase):
    ...
    def test_launcher_opens_only_packaged_world_with_r2025a(self) -> None:
        ...
        cmd = (PACKAGING / "Launch-WebeeBlocks.cmd").read_text(encoding="utf-8")
        self.assertIn("worlds\\crazyflie_runtime_v2.wbt", launcher)
        self.assertIn(".$worldBaseName.wbproj", launcher)
        self.assertIn("robotWindow:\\s+Crazyflie WebeeBlocks", launcher)
        self.assertIn("--mode=realtime", launcher)
        self.assertNotIn("--mode=pause", launcher)
        self.assertNotIn("--mode=run", launcher)
        self.assertIn("$worldArgument = '\"' + $session.World + '\"'", launcher)
        self.assertIn("Test-WebotsR2025a", launcher)
        self.assertIn("--version", launcher)
        self.assertIn("-match 'R2025a'", launcher)
        self.assertIn("Loopback, 0", launcher)
        self.assertNotIn("Start-Job", launcher)
        self.assertNotIn("Receive-Job", launcher)
        self.assertNotIn("Stop-Job", launcher)
        self.assertIn("New-WebeeBlocksRobotWindowSession", launcher)
        self.assertIn("[string]$PerspectivePath", launcher)
        self.assertIn("$sessionPerspective = Join-Path $worldRoot \".$name.wbproj\"", launcher)
        self.assertIn("[regex]::Replace(", launcher)
        self.assertIn("(?m)^robotWindow:", launcher)
        self.assertIn("La perspective de session ne peut pas neutraliser proprement l ouverture automatique de la Robot Window", launcher)
        self.assertIn("Perspective = $sessionPerspective", launcher)
        self.assertIn("Remove-Item -LiteralPath $Session.Perspective -Force", launcher)
        self.assertIn("-PerspectivePath $worldPerspective", launcher)
        self.assertIn("La perspective de session autorise encore Webots a ouvrir le navigateur par defaut", launcher)
        self.assertIn("_session_${PID}_$token", launcher)
        self.assertIn("$serverA = Start-WebeeBlocksLocalServer", launcher)
        self.assertIn("$serverB = Start-WebeeBlocksLocalServer", launcher)
        self.assertIn("if ($sessionA.Name -eq $sessionB.Name)", launcher)
        self.assertIn("if ($serverA.Port -eq $serverB.Port)", launcher)
        self.assertIn("Get-WebeeBlocksWebotsPort", launcher)
        self.assertIn("Get-WebeeBlocksRobotWindowUri", launcher)
        self.assertIn("Wait-WebeeBlocksRobotWindow", launcher)
        self.assertIn("Get-WebeeBlocksChromeLaunch", launcher)
        self.assertIn("Start-WebeeBlocksChromeWindow", launcher)
        self.assertIn("--port=$requestedPort", launcher)
        self.assertIn("--user-data-dir=", launcher)
        self.assertIn("--new-window", launcher)
        self.assertIn("while (-not $webotsProcess.WaitForExit(15))", launcher)
        self.assertIn("Connection: close", launcher)
        self.assertIn("Access-Control-Allow-Origin: *", launcher)
        self.assertIn("Cache-Control: no-store, no-cache, must-revalidate", launcher)
        self.assertIn("WEBEEBLOCKS_CHROME_EXE", cmd)
        self.assertIn("WEBEEBLOCKS_CHROME_PROFILE", cmd)
        self.assertIn("Chrome-R2025a", cmd)
        self.assertNotIn("reg.exe", cmd.lower())
        self.assertNotIn("WebeeBlocks-Chrome.cmd", cmd)
