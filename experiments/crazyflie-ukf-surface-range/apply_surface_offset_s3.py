...
def transform(original: str) -> str:
    ...
    replace_once(
        ...
        "static float surfaceAfterClearance = 0.0f;\n"
        "static float surfaceDelta = 0.0f;\n"
        "static float surfaceOffsetBefore = 0.0f;\n"
        "static float surfaceNormalWindow[5] = {0};\n"
        "static uint8_t surfaceNormalCount = 0;\n"
        "static uint8_t surfaceNormalIndex = 0;\n"
        "static float surfaceSuspectWindow[5] = {0};\n"
        "static uint8_t surfaceSuspectCount = 0;\n"
        "static uint8_t surfaceSuspectIndex = 0;\n"
        "static int8_t surfaceResidualSign = 0;\n"
        "static float surfaceStableAnchor = 0.0f;\n"
        "static uint32_t surfaceStableStartMs = 0;\n"
        ...
        "// Structural median window; settling uses sensor-reported ToF noise, not S3-A outcome.\n"
        "#define S3_CLEARANCE_WINDOW 5U\n"
        "// Fixed before S3-A from the pre-registered 4-6 s slow-motion control.\n"
        "#define S3_SETTLE_MIN_MS 1000U\n"
        "#define S3_SETTLE_SIGMA 3.0f\n"
        ...
        "};\n"
        "\n"
        "static float s3Median5(const float values[S3_CLEARANCE_WINDOW])\n"
        "{\n"
        "  float sorted[S3_CLEARANCE_WINDOW];\n"
        "  for (uint8_t i = 0; i < S3_CLEARANCE_WINDOW; i++) { sorted[i] = values[i]; }\n"
        "  for (uint8_t i = 1; i < S3_CLEARANCE_WINDOW; i++)\n"
        "  {\n"
        ...
        "    sorted[j + 1] = value;\n"
        "  }\n"
        "  return sorted[S3_CLEARANCE_WINDOW / 2U];\n"
        "}\n"
        "\n"
        "static Axis3f accAccumulator;",
        "S3 experiment state",
    ...
    reset_body = (
        ...
        "  surfaceAfterClearance = 0.0f;\n"
        "  surfaceDelta = 0.0f;\n"
        "  surfaceOffsetBefore = 0.0f;\n"
        "  memset(surfaceNormalWindow, 0, sizeof(surfaceNormalWindow));\n"
        "  surfaceNormalCount = 0;\n"
        "  surfaceNormalIndex = 0;\n"
        "  memset(surfaceSuspectWindow, 0, sizeof(surfaceSuspectWindow));\n"
        "  surfaceSuspectCount = 0;\n"
        "  surfaceSuspectIndex = 0;\n"
        "  surfaceResidualSign = 0;\n"
        "  surfaceStableAnchor = 0.0f;\n"
    ...
    replace_once(
        ...
        "              {\n"
        "                if (surfaceNormalCount < S3_CLEARANCE_WINDOW)\n"
        "                {\n"
        "                  surfaceNormalWindow[surfaceNormalCount++] = surfaceClearance;\n"
        "                }\n"
        "                else\n"
        "                {\n"
        "                  surfaceNormalWindow[surfaceNormalIndex] = surfaceClearance;\n"
        "                  surfaceNormalIndex = (surfaceNormalIndex + 1U) % S3_CLEARANCE_WINDOW;\n"
        "                }\n"
        "                surfaceBaselineClearance = (surfaceNormalCount == S3_CLEARANCE_WINDOW)\n"
        "                  ? s3Median5(surfaceNormalWindow) : surfaceClearance;\n"
        "                surfaceAfterClearance = surfaceBaselineClearance;\n"
        "                surfaceDelta = 0.0f;\n"
        "                surfaceOffsetBefore = surfaceOffset;\n"
        "                surfaceResidualSign = 0;\n"
        "                surfaceSuspectCount = 0;\n"
        "                surfaceSuspectIndex = 0;\n"
        "                surfaceStableAnchor = surfaceClearance;\n"
        "                surfaceStableStartMs = nowMs;\n"
        ...
        "              }\n"
        "\n"
        "              const float instantaneousDelta = surfaceBaselineClearance - surfaceClearance;\n"
        "              const int8_t currentResidualSign = instantaneousDelta > 0.0f ? 1 :\n"
        "                (instantaneousDelta < 0.0f ? -1 : 0);\n"
        "              if (fabsf(instantaneousDelta) >= S3_STEP_MIN_M)\n"
        "              {\n"
        "                if (surfaceResidualSign == 0)\n"
        "                {\n"
        "                  surfaceResidualSign = currentResidualSign;\n"
        "                }\n"
        "                else if (currentResidualSign != surfaceResidualSign)\n"
        "                {\n"
        "                  surfaceResidualSign = currentResidualSign;\n"
        "                  surfaceSuspectStartMs = nowMs;\n"
        "                  surfaceBaroAtSuspect = latestRelativeBaro;\n"
        ...
        "              }\n"
        "\n"
        "              const float settleLimit = S3_SETTLE_SIGMA * m.data.tof.stdDev * fabsf(dcm[2][2]);\n"
        "              if (fabsf(surfaceClearance - surfaceStableAnchor) > settleLimit)\n"
        "              {\n"
        "                surfaceStableAnchor = surfaceClearance;\n"
        "                surfaceStableStartMs = nowMs;\n"
        "                surfaceSuspectCount = 0;\n"
        "                surfaceSuspectIndex = 0;\n"
        "              }\n"
        "              if (surfaceResidualSign != 0 && currentResidualSign == surfaceResidualSign)\n"
        "              {\n"
        "                if (surfaceSuspectCount < S3_CLEARANCE_WINDOW)\n"
        "                {\n"
        "                  surfaceSuspectWindow[surfaceSuspectCount++] = surfaceClearance;\n"
        "                }\n"
        "                else\n"
        "                {\n"
        "                  surfaceSuspectWindow[surfaceSuspectIndex] = surfaceClearance;\n"
        "                  surfaceSuspectIndex = (surfaceSuspectIndex + 1U) % S3_CLEARANCE_WINDOW;\n"
        "                }\n"
        "              }\n"
        "              surfaceAfterClearance = (surfaceSuspectCount == S3_CLEARANCE_WINDOW)\n"
        "                ? s3Median5(surfaceSuspectWindow) : surfaceClearance;\n"
        "              const float deltaSurface = surfaceBaselineClearance - surfaceAfterClearance;\n"
        "              surfaceDelta = deltaSurface;\n"
        "              surfaceCandidate = surfaceOffsetBefore + deltaSurface;\n"
        "              surfaceBaroDelta = latestRelativeBaro - surfaceBaroAtSuspect;\n"
        "              const uint32_t suspectAgeMs = nowMs - surfaceSuspectStartMs;\n"
        "              const bool sameSignPersistent = surfaceResidualSign != 0 &&\n"
        "                currentResidualSign == surfaceResidualSign && suspectAgeMs >= S3_SUSPECT_MIN_MS;\n"
        "              const bool settled = surfaceSuspectCount == S3_CLEARANCE_WINDOW &&\n"
        "                (nowMs - surfaceStableStartMs) >= S3_SETTLE_MIN_MS;\n"
        ...
        "              }\n"
        "              else if (!sameSignPersistent || !settled)\n"
        "              {\n"
        ...
        "                flowActive = true;\n"
        "                surfaceBaselineClearance = surfaceAfterClearance;\n"
        "                for (uint8_t i = 0; i < S3_CLEARANCE_WINDOW; i++)\n"
        "                {\n"
        "                  surfaceNormalWindow[i] = surfaceAfterClearance;\n"
        "                }\n"
        "                surfaceNormalCount = S3_CLEARANCE_WINDOW;\n"
        "                surfaceNormalIndex = 0;\n"
        "                surfaceSuspectCount = 0;\n"
        "                surfaceSuspectIndex = 0;\n"
        "                surfaceResidualSign = 0;\n"
    ...
    replace_once(
        ...
        "LOG_ADD(LOG_FLOAT, surfBefore, &surfaceBaselineClearance)\n"
        "LOG_ADD(LOG_FLOAT, surfAfter, &surfaceAfterClearance)\n"
        "LOG_ADD(LOG_FLOAT, surfDelta, &surfaceDelta)\n"
        "LOG_ADD(LOG_INT8, surfSign, &surfaceResidualSign)\n"
        "LOG_ADD(LOG_UINT8, surfN, &surfaceSuspectCount)\n"
