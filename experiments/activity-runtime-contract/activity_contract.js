...
(function(root, factory) {
  ...
  'use strict';
  const BACKEND_ACTION_KINDS = new Set(['takeoff','move','vertical','turn','wait','set_speed','land']);
  ...
  function preflightBackend(profile,facts,backend){const caps=backend&&backend.capabilities;if(!caps)fail('backend capabilities unavailable');const actions=new Set(caps.actions||[]);for(const kind of facts.blocks)if(BACKEND_ACTION_KINDS.has(kind)&&!actions.has(kind))fail('backend action capability unavailable: '+kind);const ranges=new Set(caps.rangeDirections||[]);for(const d of facts.ranges)if(!ranges.has(d))fail('backend range capability unavailable: '+d);return true;}
  function toolboxXml(profile){return '<xml>'+profile.toolbox.map(type=>'<block type="'+type+'"></block>').join('')+'</xml>';}
  function execute(profile,ws,compiler,interpreter,backend){preflightWorkspace(profile,ws);applyFieldBounds(profile,ws);const ast=compiler.compileWorkspace(ws);const facts=preflightAst(profile,ast);preflightBackend(profile,facts,backend);interpreter.run(ast,backend);return ast;}
  return{preflightWorkspace,applyFieldBounds,preflightAst,preflightBackend,toolboxXml,execute};
});
