(function(root, factory) {
  ...
  function fail(message) {
    throw new Error('activity profile: ' + message);
  }
  ...
  function validateBounds(bounds, path) {
    ...
    if (bounds.min > bounds.max)
      fail(path + '.min must be <= max');
    if (bounds.step <= 0)
      fail(path + '.step must be > 0');
  }
  ...
  function validateProfile(profile, blockCatalog) {
    ...
    if (!isObject(profile.brief) || typeof profile.brief.visible !== 'boolean')
      fail('brief.visible must be a boolean');
    ...
    if (!Array.isArray(profile.toolbox) || profile.toolbox.length === 0)
      fail('toolbox must be a non-empty array');
    ...
    profile.toolbox.forEach(function(type) {
      ...
      if (!Object.prototype.hasOwnProperty.call(blockCatalog, type))
        fail('unknown block type: ' + type);
      if (seen[type])
        fail('duplicate block type: ' + type);
      seen[type] = true;
    });

    var parameterBounds = profile.parameterBounds || {};
    if (!isObject(parameterBounds))
      fail('parameterBounds must be an object');
    Object.keys(parameterBounds).forEach(function(blockType) {
      if (!seen[blockType])
        fail('parameter bounds declared for hidden block: ' + blockType);
      var fields = parameterBounds[blockType];
      if (!isObject(fields))
        fail('parameterBounds.' + blockType + ' must be an object');
      Object.keys(fields).forEach(function(fieldName) {
        validateBounds(fields[fieldName], 'parameterBounds.' + blockType + '.' + fieldName);
      });
    });

    if (!Array.isArray(profile.hardware))
      fail('hardware must be an array');

    if (!isObject(profile.timer) || typeof profile.timer.enabled !== 'boolean')
      fail('timer.enabled must be a boolean');
    ...
    if (!Array.isArray(profile.runtime.allowedStatementKinds))
      fail('runtime.allowedStatementKinds must be an array');
    if (!Array.isArray(profile.runtime.rangeDirections))
      fail('runtime.rangeDirections must be an array');
  ...
  function resolveProfile(profile, blockCatalog) {
    ...
    resolved.toolbox = resolved.toolbox.map(function(type) {
      ...
      if (resolved.parameterBounds[type])
  ...
  function resolveById(document, profileId, blockCatalog) {
    if (!isObject(document) || !Array.isArray(document.activities))
      fail('activity document must contain an activities array');
    var matches = document.activities.filter(function(profile) {
      return profile.id === profileId;
    });
    if (matches.length !== 1)
      fail('activity id must resolve exactly once: ' + profileId);
