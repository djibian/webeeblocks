(function(root, factory) {
  ...
  function compileExpression(block) {
    switch (block.type) {
      case 'webeeblocks_v2_range': {
        var direction = String(field(block, 'DIRECTION'));
        if (SENSOR_DIRECTIONS.indexOf(direction) < 0) fail('unsupported range direction: ' + direction);
        return {kind: 'range', direction: direction, unit: 'm'};
      }
      case 'math_number': return {kind: 'number', value: finite(field(block, 'NUM'), 'number')};
      case 'logic_compare': {
        var compare = String(field(block, 'OP'));
        if (['LT', 'LTE', 'GT', 'GTE', 'EQ', 'NEQ'].indexOf(compare) < 0) fail('unsupported comparison: ' + compare);
        return {kind: 'compare', op: compare, left: valueChild(block, 'A'), right: valueChild(block, 'B')};
      }
      case 'logic_operation': {
        var logic = String(field(block, 'OP'));
        if (logic !== 'AND' && logic !== 'OR') fail('unsupported logic operation: ' + logic);
        return {kind: 'logic', op: logic, left: valueChild(block, 'A'), right: valueChild(block, 'B')};
      }
  ...
  function repeatCount(block) {
    if (typeof block.getInputTargetBlock === 'function' && block.getInputTargetBlock('TIMES')) {
      var expression = valueChild(block, 'TIMES');
      if (expression.kind !== 'number' || Math.floor(expression.value) !== expression.value) fail('repeat count must be an integer literal');
  ...
  function compileStatement(block) {
    switch (block.type) {
      ...
      case 'webeeblocks_turn': return {kind: 'turn', angle_deg: bounded(field(block, 'ANGLE'), 'angle_deg', {min: -179, max: 179})};
      ...
      case 'webeeblocks_v2_move': {
        var direction = String(field(block, 'DIRECTION'));
        if (['forward', 'back', 'left', 'right'].indexOf(direction) < 0) fail('unsupported move direction: ' + direction);
        return {kind: 'move', direction: direction, distance_m: bounded(field(block, 'DISTANCE'), 'distance_m', LIMITS.distance_m)};
      }
      case 'webeeblocks_v2_vertical': {
        var verticalDirection = String(field(block, 'DIRECTION'));
        if (verticalDirection !== 'up' && verticalDirection !== 'down') fail('unsupported vertical direction: ' + verticalDirection);
        return {kind: 'vertical', direction: verticalDirection, distance_m: bounded(field(block, 'DISTANCE'), 'vertical_m', LIMITS.vertical_m)};
      }
      case 'webeeblocks_v2_turn': {
        var turnDirection = String(field(block, 'DIRECTION'));
        var degrees = bounded(field(block, 'ANGLE'), 'angle_deg', {min: 1, max: 179});
        if (turnDirection !== 'left' && turnDirection !== 'right') fail('unsupported turn direction: ' + turnDirection);
        return {kind: 'turn', angle_deg: turnDirection === 'left' ? degrees : -degrees};
      }
      ...
      case 'controls_if': {
        if (block.elseifCount_ && block.elseifCount_ !== 0) fail('else-if branches are not part of AST v1');
        var result = {kind: 'if', condition: valueChild(block, 'IF0'), then: statementChildren(block, 'DO0')};
        if (block.getInputTargetBlock('ELSE')) result.else = statementChildren(block, 'ELSE');
        return result;
      }
  ...
  function compileSequence(first) {
    var result = [], block = first, guard = 0;
    while (block) {
      guard += 1;
  ...
  function compileWorkspace(workspace) {
    ...
    if (tops.length !== 1) fail('Crazyflie program must have exactly one top-level sequence');
    var program = compileSequence(tops[0]);
  ...
  return {LIMITS: LIMITS, SENSOR_DIRECTIONS: SENSOR_DIRECTIONS, compileExpression: compileExpression, compileStatement: compileStatement, compileSequence: compileSequence, compileWorkspace: compileWorkspace};
});
