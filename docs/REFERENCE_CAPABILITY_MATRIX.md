...
| Multi-ranger directional distance | Multi-ranger deck | `webeeblocks_v2_range` → `range(direction)`; AST vocabulary has front/back/left/right/up | Current WWI backend advertises and reads front/back/left/right/up through dedicated Webots sensors | Integrated #362 consumes the exact teacher-bound shared Runtime `range(front/back/left/right/up)` semantics inside the trusted production host using fresh same-epoch Multi-ranger observations. Range data remains non-authority; interpreter branch/repeat/variable progression stays host-owned, later effects retain their independent authority/acknowledgement/completion chain, and post-takeoff observation/interpreter uncertainty is terminal fail-closed. This is deterministic fake/injected-hardware proof, not real-device qualification | Progression 3 exposes front; broad reactive profile declares front/back/left/right/up | **covered** in simulation; **deterministic physical path integrated; real-device unproven** |
...
| Multi-ranger upward range | Multi-ranger deck | Generic AST already admits `up` | Current WWI backend advertises and reads `up` through a dedicated upward Webots distance sensor | Integrated #362 includes `up` in the same exact teacher-bound shared Runtime path, with a fresh same-epoch non-authority observation and the same fail-closed interpreter/effect boundaries as the other supported Multi-ranger directions. No real-device qualification is claimed | Broad reactive profile declares `up` | **covered** in simulation; **deterministic physical path integrated; real-device unproven** |
...
freshness and direct readback, and #362 binds the exact teacher-authorized shared
Runtime interpreter to fresh same-epoch Multi-ranger observations inside that
same trusted host.
...
authorization -> watchdog activation/liveness -> causal takeoff -> shared Runtime
execution of zero or more exact teacher-bound horizontal move/yaw-turn/vertical/
bottom-light effects, bounded no-effect waits or `set_speed` state transitions,
and `range(front/back/left/right/up)` observations that may drive normal
interpreter control flow -> exact terminal controlled landing from the trusted
final nominal altitude -> fresh same-epoch finished/non-flying/high-level-inactive
completion. Every effect boundary and no-effect step reconstructs the required
host-owned authority/provenance; range observations remain non-authority and do
not mint effect permission. Caller-selected motion/light parameters, speed/wait
values, cursor/completion or caller-returned provenance do not become authority.
Ambiguous acknowledgement, transport, observation, interpreter, completion or
...
It does **not** qualify real flight. Multi-ranger value consumption, like the
movement, timing and bottom Color LED paths, now has deterministic trusted-host
coverage but remains real-device unqualified. The available real-device evidence
from the superseded #226 checkpoint established only bounded hardware
identity/presence observations and cannot be reused as execution qualification.
...
qualification of the integrated physical envelope**, not another missing generic
student capability or host-composition slice. Preserve the existing trusted-host,
teacher, preflight, watchdog, failsafe and fail-closed boundaries when preparing
that qualification; do not infer physical acceptance from deterministic
composition proof.
