...
| Bottom Color LED Deck light/color | Bottom-mounted Color LED Deck | `webeeblocks_v2_light` → `set_light(color)` with a bounded generic palette | Runtime v2 exposes the action through WWI on an attached bottom-deck envelope with side-visible diffuser and LED-driven nearby halo; fixed top/three-quarter/side R2025a render evidence is integrated | Integrated #341 consumes the exact teacher-bound `set_light(color)` statement through a narrow one-shot `colorLedBot.wrgb8888` PARAM transport. The trusted host resolves only the exact live writable `uint32_t` entry, requires the same teacher/session/watchdog/SafeLink/current-program/effect-exclusion chain, establishes a unique receive-order fence before accepting the exact write reply, removes fallible write-listener teardown before minting completion authority, and requires a fresh same-epoch exact-value readback before returning to `FLYING` and advancing the sequence. Caller-selected color/value/parameter/id/completion data remains rejected. This is deterministic fake/injected-hardware evidence only; no real-device qualification is claimed | Broad reactive profile only | **covered** in simulation; **deterministic physical path integrated; real-device unproven** |
...
#341 adds the exact bottom Color LED effect with one-shot PARAM write, causal
reply ordering and fresh same-epoch readback under the same trusted run.
...
boundary and no-effect step reconstructs the required host-owned
authority/provenance; caller-selected motion/light parameters, speed/wait values,
cursor/completion or caller-returned provenance do not become authority. Ambiguous
...
vertical + bottom Color LED + bounded no-effect wait/set-speed state +
