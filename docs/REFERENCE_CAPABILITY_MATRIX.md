...
| Vertical movement | Crazyflie 2.1 airframe / flight control | `webeeblocks_v2_vertical` → `vertical(up/down, distance)` | Current WWI backend advertises and executes up/down with bounded altitude targets and stable-position completion | Physical student backend not proven | Broad reactive profile only | **covered** in simulation for the generic vertical vocabulary; **physical unproven** |
