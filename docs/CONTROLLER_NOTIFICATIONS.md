...
- `x3-independent-props-off` — checkpoint-only #70 X3 characterization with props removed, backed by the exact `WebeeBlocks-X3-Characterization` bundle. Preparation is restore-and-verify only for the cached #251 firmware, pinned cflib and four-wheel offline runtime; the bundle includes the reviewed measured-guide/independent-clock witness procedure and creates no motorized or execution authority.
