...
nominal altitude, #341 consumes exact teacher-bound bottom Color LED effects
through one-shot PARAM writes with same-epoch freshness and exact readback, and
#362 integrates the exact teacher-bound shared Runtime interpreter with fresh
same-epoch Multi-ranger `front/back/left/right/up` observations inside the trusted
host. The supported deterministic physical envelope is therefore causal takeoff
-> shared Runtime execution of zero or more exact horizontal move/turn/vertical/
bottom-light effects, bounded no-effect wait/set-speed transitions and fresh
Multi-ranger observations that may drive normal interpreter control flow ->
terminal controlled land, under the integrated teacher, session, watchdog,
supervisor, SafeLink, acknowledgement, exclusion and current-program trust chain.
...
real-device qualification of that integrated envelope, not another missing
student-facing generic capability or host-composition slice.
...
- established deterministic physical envelope: #266/#267/#268/#271/#272/#273/
  ...
  exact-value readback before restoring `FLYING`; and #362 integrates the exact
  teacher-bound shared Runtime interpreter with fresh same-epoch Multi-ranger
  `front/back/left/right/up` observations, preserving host-owned control flow and
  independent authority/completion for later effects. Unknown acknowledgement/
  effect/observation/interpreter/completion outcome remains fail-closed and cannot
  authorize retry or reset
- remaining boundary: representative real-device qualification of the complete
  integrated deterministic envelope. No currently justified generic student
  capability remains missing from deterministic simulation↔physical continuity.
  Bottom Color LED and Multi-ranger range-value consumption now both have
  deterministic trusted-host paths, but remain real-device unqualified. The
  ...
  physical run still requires a new #267 teacher authorization. Deterministic
  host evidence is not a real-flight qualification and authorizes no motorized
...
- established prerequisite and deterministic subset: #193/#196 plus
  ...
  freshness/readback; #362 integrates fresh same-epoch Multi-ranger observations
  with the exact teacher-bound shared Runtime interpreter; and #305/#308 compose
  exact terminal controlled landing from the final trusted altitude plus fresh
  non-flying completion. The browser-facing surface remains non-authority with
  `executionAuthority:false`, and the cflib close path retains its safety-zero
  transport setpoint. This established subset is deterministic fake/injected-
  hardware integration evidence, not real-flight qualification
...
- depends: representative real-device qualification of the integrated physical
  ...
  #276/#308/#341 effect substrate and #362 shared-interpreter/range-observation
  continuity. Bottom Color LED and Multi-ranger value consumption have
  deterministic host integration but no real-device qualification. Stock auto-
  arming can return the vehicle to ReadyToFly after the landing/reset cycle, so a
  host disarm request is not a durable post-mission lockout. The pinned watchdog
  has no disable/reset command after first activation, so its trusted keepalive
  ...
  until explicit STM+deck reset is separately proven, followed by a new
  connection epoch and complete re-preflight. That reset is a motor-cut recovery
  boundary, not a normal abort action, and must never be triggered
...
- proof direction: preserve backend-neutral AST continuity and the integrated
  trusted-host/teacher/preflight/watchdog/failsafe boundaries while qualifying a
  representative real device. Deterministic composition does not authorize a
  motorized run or substitute for physical acceptance. Any later exact real-
  device protocol must keep #257 as the exclusive authoritative supervisor
  ...
  outcome remains ambiguous and must not trigger automatic command resend.
  Immediate emergency stop and high-level commander stop stay exceptional because
  ...
  #267 teacher authorization even if stock auto-arming returns ReadyToFly
